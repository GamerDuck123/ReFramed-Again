...
public class ReFramedSlabsHalfLayerBlock extends HalfLayerDoubleReFramedBlock {
    ...
    public ReFramedSlabsHalfLayerBlock(Properties settings) {
        super(settings);
    }

    @Override
    public VoxelShape getShape(BlockState state, BlockGetter world, BlockPos pos, CollisionContext context) {
        return getSlabsHalfLayerShape(state.getValue(EDGE), state.getValue(EDGE_FACE), state.getValue(LAYERS));
    }
